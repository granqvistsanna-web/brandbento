...
must_haves:
  truths:
    ...
    - "User can toggle tile background between white, dark, brand primary"
  artifacts:
...
1. Create `src/components/tiles/LogoTile.tsx`:
   ...
   export function LogoTile({ onClick, isEditing }: LogoTileProps) {
     ...
     // Calculate adaptive background when set to 'auto' (for future use)
     // Currently uses user-selected background, but luminance hook available
...
2. Add CSS (in component or global):
  ...
  <done>
    LogoTile displays logo centered with 24px padding, adaptive background via useImageLuminance, variant filters, scale transform
  </done>
</task>
...
3. Create `src/components/panels/index.ts`:
  ...
  <done>
    LogoEditPanel with file upload (LOGO-03), scale slider (LOGO-05), variant toggle (LOGO-04), background toggle (LOGO-06), all wired to state
  </done>
</task>
...
2. Background adapts to luminance when useImageLuminance returns value
...
6. Background toggle switches between white, #111, brand primary (LOGO-06)
7. Slider creates single undo step per drag (not multiple)
</verification>

<success_criteria>
- LOGO-01: Logo displays centered with minimum 24px padding
- LOGO-02: Logo background auto-adapts based on luminance (hook available)
...
- LOGO-06: User can toggle tile background (white, dark, brand primary)
</success_criteria>
