   ...
   export const useCanvasStore = create<CanvasStore>()(
     temporal(
       subscribeWithSelector((set, get) => ({
         ...
         setLogoBackground: (background: 'white' | 'dark' | 'primary') => set((state) => ({
           tileSettings: {
             ...state.tileSettings,
...
3. Update CanvasStore interface with new actions:
   ...
   interface CanvasStore extends CanvasState {
     ...
     setLogoBackground: (background: 'white' | 'dark' | 'primary') => void;
  ...
  <verify>
    ...
    Create simple test: import useTemporalStore, call undo() - no runtime errors
  </verify>
  <done>
    Zustand store wrapped with temporal middleware, 30-step undo history, partialize excludes transient state, useTemporalStore hook exported
  </done>
</task>
...
4. Undo works: Import useTemporalStore, call undo(), verify state reverts
5. Luminance works: Hook returns 'light' or 'dark' for test images
</verification>
