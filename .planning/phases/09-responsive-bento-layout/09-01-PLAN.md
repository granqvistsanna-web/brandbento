...
must_haves:
  truths:
    ...
    - "User's preset and density preferences persist across sessions"
    - "Grid height fills viewport correctly on all devices"
  artifacts:
...
<task type="auto">
  ...
  <action>
Create TypeScript types for the layout system in src/types/layout.ts:
- BreakpointName: 'mobile' | 'tablet' | 'desktop'
- DensityMode: 'cozy' | 'dense'
- LayoutPresetName: 4 preset names
- TileSpan, BreakpointSpans, LayoutPreset, LayoutState interfaces

Create src/config/layoutPresets.ts with:
- BREAKPOINTS: mobile=0, tablet=768, desktop=1024
- 4 LAYOUT_PRESETS (balanced, heroLeft, heroCenter, stacked) each defining:
  - columns per breakpoint (mobile=2, tablet/desktop=4)
  - rows per breakpoint (3 rows for tablet/desktop)
  - gap values (cozy=16px, dense=8px)
  - rowHeight values (cozy=280px, dense=200px)
  - tileSpans mapping tile types to colSpan/rowSpan per breakpoint

Tile types: hero, logo, colors, primaryType, secondaryType, imagery, uiPreview, editorial, product, utility

Reference 09-RESEARCH.md for recommended span configurations per preset.
  </action>
  <verify>
