...
must_haves:
  truths:
    ...
    - "Undo history is limited to 30 entries"
  artifacts:
    ...
    - path: "src/store/useBrandStore.js"
      provides: "History limited to 30 entries and resetToDefaults action"
  key_links:
...
<objective>
Install dependencies, create keyboard shortcuts hook, build the floating Toolbar component with auto-hide, and add history depth limit to store.

Purpose: Foundation for all Phase 6 workflow features - installs html-to-image for export, react-hot-toast for notifications, wires Cmd+Z/Cmd+Shift+Z shortcuts, creates the floating toolbar UI, and ensures undo history stays bounded (TOOL-01, TOOL-02, TOOL-03, TOOL-05).

Output: Working keyboard shortcuts, installed dependencies, floating toolbar with auto-hide behavior, history limited to 30 entries.
</objective>
...
<task type="auto">
  <name>Task 2: Add history depth limit and resetToDefaults action to store</name>
  <files>src/store/useBrandStore.js</files>
  <action>
Update useBrandStore.js to:

1. Add a MAX_HISTORY_DEPTH constant at the top of the file:
```javascript
const MAX_HISTORY_DEPTH = 30;
```

2. Create a helper function to push history with depth limiting (add before the create() call):
```javascript
const pushHistory = (currentState, history) => {
  const newPast = [...history.past, { brand: currentState.brand, tiles: currentState.tiles }];
  // TOOL-05: Limit undo stack to 30 entries
...
3. Update all places that push to history.past to use the helper:
- `loadPreset`: Replace manual history push with `pushHistory(get(), history)`
- `applyPalette`: Replace manual history push with `pushHistory(get(), history)`
- `swapTileType`: Replace manual history push with `pushHistory(get(), history)`
- `updateBrand`: Replace manual history push with `pushHistory(get(), history)`
- `updateTile`: Replace manual history push with `pushHistory(get(), history)`
- `redo`: This one also pushes to past, update it too

4. Add a new `resetToDefaults` action that loads DEFAULT_BRAND and INITIAL_TILES but IS undoable:
```javascript
resetToDefaults: () => {
  ...
  set({
    brand: DEFAULT_BRAND,
    tiles: INITIAL_TILES,
    history: pushHistory({ brand, tiles }, history),
  });
},
```

This is different from loadRandomTemplate which clears history. resetToDefaults preserves undo capability (TOOL-08).
  </action>
  <verify>
1. Verify MAX_HISTORY_DEPTH = 30 is defined
2. Verify pushHistory helper exists and enforces the limit
3. Verify resetToDefaults action exists and calls pushHistory (not clearing history)
4. Test: Make 35 changes, verify only 30 undo steps are available
  </verify>
  <done>History is limited to 30 entries, resetToDefaults action exists and is undoable</done>
</task>

<task type="auto">
  <name>Task 3: Create useKeyboardShortcuts hook</name>
...
<task type="auto">
  <name>Task 4: Create Toolbar component with auto-hide</name>
...
<task type="auto">
  <name>Task 5: Wire keyboard shortcuts and Toolbar in App.tsx</name>
...
10. Make 35 changes, verify undo stops after 30 steps (TOOL-05)
</verification>

<success_criteria>
- [ ] html-to-image@1.11.13 installed
- [ ] react-hot-toast installed
- [ ] MAX_HISTORY_DEPTH = 30 enforced in store
- [ ] resetToDefaults action exists and is undoable
